								...
								<c:when test="${not empty userList}">
									...
									<c:forEach items="${userList}" var="user" varStatus="vs">
										...
										<tr>
											...
											<td class="center">${user.COMPANY_NAME }</td>
											<td class="center">${user.DEPARTMENT_NAME }</td>
											<td class="center">${user.PHONE }</td>
											<td class="center">${user.TEL}</td>
											<%-- <td class="center">${user.ROLE_NAME }</td> --%>
											<td class="center">${user.EMAIL }<%-- <a title="发送电子邮件" style="text-decoration:none;cursor:pointer;" <c:if test="${QX.email == 1 }">onclick="sendEmail('${user.EMAIL }');"</c:if>>&nbsp;<i class="ace-icon fa fa-envelope-o"></i></a> --%></td>
											<%-- 
					...
					<table style="width:100%;">
						<tr>
							<td style="vertical-align:top;">
								...
								<!-- <a class="btn btn-sm" onclick="importData();">导入</a> -->
								</c:if>
...
function viewUser(USERNAME){
    ...
    function importData(){
	     ...
	     diag.Drag=true;
	     diag.Title ="导入";
	     diag.URL = '<%=basePath%>user/goImport.do';
	     diag.Width = 500;
	     diag.Height = 500;
	     diag.CancelEvent = function(){ //关闭事件
		  diag.close();
